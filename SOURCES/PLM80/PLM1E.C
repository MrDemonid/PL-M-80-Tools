...
static byte stkBlocks[20];      /* стек блоков кода (PROC, LOOP, WHILE, FOR , CASE) */
static bool b9A13[20];
static word procInfoStack[20];
static word hNodes[20];         /* стек меток начала цикла */
static word eNodes[20];         /* стек меток конца цикла */
static word nNodes[20];         /* стек меток для возможного CONTINUE */
...
static void PushControl(byte arg1b)
{
    if (controlSP == 0x13)
        ...
        stkBlocks[controlSP = controlSP + 1] = arg1b;
...
*/
static void ParseEndFOR()
{
    word p, q, r, s, t, hLab, eLab, w;

    p = procInfoStack[controlSP];
    hLab = hNodes[controlSP];
    eLab = eNodes[controlSP];
    if ((w = w9A9F[controlSP]) == 0)
    {
        /* создаем метку для возможного CONTINUE */
        WrTx2Item1Arg(T2_LOCALLABEL, nNodes[controlSP]);
        curInfoP = p;
        if (GetType() == BYTE_T)
        ...
        if (GetType() == BYTE_T) {
            ...
            q = WrTx2Item2Arg(T2_JNZ, hLab, 3);
        } else {
            r = WrTx2Item2Arg(T2_ADDW, Sub_42EF(r), Sub_42EF(s));
            q = WrTx2Item2Arg(T2_COLONEQUALS, Sub_42EF(q), Sub_42EF(r));
            q = WrTx2Item1Arg(T2_JNC, hLab);
        }
    } else
        q = WrTx2Item1Arg(T2_JMP, w);
    q = WrTx2Item1Arg(T2_LOCALLABEL, eLab);
}
...
static void IterativeDoStatement()
{
    ...
    /*
      создаем метку для возможного оператора CONTINUE
    */
    nNodes[controlSP] = NewLocalLabel();
...
/*
  Ищет в стеке блоков кода вхождения WHILE или FOR, принадлежащие
  текущей PROC.
*/
unsigned int FindControl(void)
{
    unsigned int curSP;
    ...
    while (curSP > 0)
    {
        if (stkBlocks[curSP] == DO_PROC)
        {
            /*
              если добрались до начала процедуры, то дальше не ищем
            */
            break;
        }
...
/*
  Подменяем BREAK на <JMP конец_текущего_блока>
*/
static void ParseBREAK()
{
    unsigned int cycleSP;
    ...
    {
        // подставляем джамп
        WrTx2Item1Arg(T2_JMP, eNodes[cycleSP]);
    } else {
        // мы вне блока, это не допустимо
        FatalError(ERR220);
    }
}
...
/*
  Подменяем CONTINUE на <JMP следующая_итерация>
*/

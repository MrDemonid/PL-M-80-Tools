...
void FatalError(byte errNum)
{
    switch (ovl) {
    case 255: FatalError_main(errNum); break;
    case 0:   FatalError_ov0(errNum); break;
...
{
    osfile_t fname;
    if (MapFile(&fname, isisPath) != ERROR_SUCCESS)
        return NULL;
    return fopen(fname.name, mode);
}

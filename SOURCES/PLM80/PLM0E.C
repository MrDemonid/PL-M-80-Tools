...
static void WrNestedExpression()
{
    ...
    while (1) {
        ...
        } else if (tokenType == T_SEMICOLON || (tokenType >= T_CALL && tokenType <= T_RETURN) || tokenType == T_BREAK || tokenType == T_CONTINUE) {
...
void ParseExpresion(byte endTok)
{
    ...
    while (tokenType != endTok && tokenType != T_SEMICOLON) {
        if (T_CALL <= tokenType && tokenType <= T_RETURN || tokenType == T_BREAK || tokenType == T_CONTINUE)
            break;
        WrLexToken();
        Yylex();
    }

...
static byte startLexCodeMap[] = {
    ...
    L_PROCEDURE, L_RETURN, L_BREAK, L_CONTINUE };
...
/* parse start of statement
    ...
    L_BREAK             14          break statement
    L_CONTINUE          15          continue statement
*/
...
static void ParseStartStmt()
{
    ...
    else if (stmtStartToken == T_BREAK)
        stmtStartCode = 14;
...
static bool IsEndOfModule()
{
    ...
    {
        /*
          достигли конца файла или встретили EOF,
          не найдя метки завершения программы (END programm)
        */
        WrByte(L_END);          // write closing END to lex stream
        return true;            // no more processing
    } else {
        /*
          достигли метки END programm,
          завершаем программу
        */
        Yylex();
...
{   /* process CASE statements */
    ...
    {
        /*
          начинаем очередной блок текущего CASE
        */
        NewLocalLabel();            // создаём метку начала блока
        WrByte(L_CASELABEL);
        WrWord(localLabelCnt);
        PushStateByte(15);          // завершаем текущий блок и перезапускаем State14()
        PushStateByte(11);          // парсим тело блока

    } else {
        /*
          завершаем текущий CASE
        */
        ...
        /*
          теперь создаём метку за пределами CASE,
          куда выходят блоки оператора
        */
        WrByte(L_LOCALLABEL);
        WrWord(stateWord);
...
/*
  формирует выход из блока CASE и возвращается
  к дальнейшему разбору блоков CASE (в State14())
*/
static void State15()
{
    word stateWord;

    PopStateWord(&stateWord); // получаем метку за пределами CASE
    WrByte(L_JMP);            // создаем переход на неё
    WrWord(stateWord);
    PushStateWord(stateWord); // ложим метку обратно в стек, для следующих блоков текущего CASE
    PushStateByte(14);        // переходим к разбору очередного блока текущего CASE
}

...
static byte tmpOBJECT;


static void MakeT2Error(word arg)
{
    word p;
    ...
    while (curInfoP != 0) {
        if (GetType() == STRUCT_T) {
            ...
            if ((s = q + r) < r)    /* add in the new element */
                MakeT2Error(ERR208);   /* LIMIT EXCEEDED: structure Size() */
...
static void AllocVarAddress(wpointer arg1wP)
{
    ...
    *arg1wP += GetVarSize();        /* reserve it's space */
    if (*arg1wP < GetVarSize())
        MakeT2Error(ERR207);   /* LIMIT EXCEEDED: SEGMENT Size() */
}
    ...
    while (curInfoP != 0) {
            ...
            || GetType() == PROC_T || GetType() == LABEL_T)
        {
            if (TestInfoFlag(F_EXTERNAL))
            {
                ...
                if (externalsCnt == 0)
                    MakeT2Error(ERR219);   /* LIMIT EXCEEDED: NUMBER OF EXTERNALS > 255 */
                SetLinkVal(0);
                standAlone = false;
            }
...
static void  ChkAbsoluteData()
{
    ...
    if (atFData.var.infoOffset == 0)
        ...
        i = 0;
        MakeT2Error(ERR211);   /* INVALID IDENTIFIER IN 'at' RESTRICTED REFERENCE */
    }
        ...
        i = 0;
        MakeT2Error(ERR212);   /* INVALID RESTRICTED REFERENCE IN 'at' , BASE ILLEGAL */
    }
        ...
        if (!TestInfoFlag(F_MEMBER)) {
            ...
            case 1:
                if (TestInfoFlag(F_PUBLIC))
                    MakeT2Error(ERR178); /* INVALID 'at' RESTRICTED REFERENCE, external
...
/*
  в файле PLMAT.TMP найдены данные с префиксом AT (ADDR)
*/
static void ParseAbsoluteData()
{
    ...
    if (atFData.var.infoOffset != 0)
    {
        ...
        if (TestInfoFlag(F_AT))
            if (curInfoP >= atFData.infoP)
                MakeT2Error(ERR213);   /* UNDEFINED RESTRICTED REFERENCE IN 'at' */
    }
    ChkAbsoluteData();
}
...
static void ProcAtFile()
{
    ...
    while (1)
    {
        ...
        switch (atFData.type)
        {
            case 0: ParseAbsoluteData(); break;                /* AT_AHDR */
...
static void CreateNMSfile()
{
    if ((tmpOBJECT = OBJECT || PRINT) || IXREF)
    {
        OpenF(&nmsFile, 3);

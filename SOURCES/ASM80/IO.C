...
static word ParseIsisName(spath_t *pInfo, const char *isisPath)
{
    ...
    {
        for (i = 0; i < MAX_FILE_NAME && isalnum(*isisPath); i++)
...
static word MapFile(osfile_t *osfileP, const char *isisPath)
{
    ...
    if (info.deviceType == 3) { // add file name
        ...
        int i;
        for (i = 0; i < MAX_FILE_NAME && info.name[i]; i++)
...
void Rename(byte *oldP, byte *newP, word *statusP)
{
...
void Spath(byte *pathP, spath_t *infoP, word *statusP)
{

...
static word *fixupRecLenPtrs[] = {&rPublics.len, &rInterseg.len, &rExtref.len, &rContent.len};
...
void WriteRec(byte *recP)
{
    ...
    recLen = ++*(word *)lenP + 3;    /* include crc byte + type + len word */
    crc = 0;            /* crc */
    lenP--;
    for (i = 2; i <= recLen; i++)
        crc -= *lenP++;
...
static void AddFixupRec()
{
    ...
    word *dtaP;                              // to check doesn't conflict with plm global usage
...
void WriteExtName()
{
    ...
    if (rExtnames.len + 9 > 125) {   /* check room for name */
        WriteRec((byte *)&rExtnames);    /* flush existing extNam Record() */
    ...
    for (i = 0; i <= nameLen; i++)            /* and name */
        rExtnames.dta[extNamIdx + i] = tokName[i];
...
/*
  добавляет имя в список (public или locals) для последующей записи в OBJ-файл
  recSymP - указатель на буфер с именами
*/
void AddSymbol()
{
    ...
    // добавляем относительный адрес (от начала кода текущего файла)
    *(word *)recSymP = tokenSym.curP->offset;
    // теперь длину и само публичное имя
    UnpackToken(tokenSym.curP->tok, (dtaP = (recSymP += 2) + 1));
    dtaP[MAXSYMSIZE] = ' ';     /* ограничиваем длину имени */
    *recSymP = 0;               /* длина имени */
    /*
      обрезаем пробелы, оставляя только имя
    */
    while (dtaP[0] != ' ')
    {
        ++*recSymP;
...
static void WriteSymbols(byte isPublic) /* isPublic= true -> PUBLICs else LOCALs */
{
    ...
    for (segId = 0; segId <= 4; segId++)
    {
        ...
        while (++tokenSym.curP < endSymTab[TID_SYMBOL])
        {
            /* make sure there is room offset, len, symbol, 0 */
            if (recSymP > &rPublics.crc - (MAXSYMSIZE + 4))
...
void WriteModhdr()
{
    ...
    *(word *)dtaP = 0;    /* the trnId & trnVn bytes */

...
static void PrintStr(byte *str)
{
    while (*str != 0)
        PrintChar(*str++);
}

static void PrintNStr(byte cnt, byte *str)
{
    while (cnt-- > 0)
        PrintChar(*str++);
}
...
static void Itoa(word n, byte *buf)
{
...
static void OutNStr(word cnt, byte *s)
{
    while (cnt-- > 0)
        Outch(*s++);
}

...
static byte *AddMacroText(byte *lowAddr, byte *highAddr)
{
...
void GetMacroToken()
{
    ...
    if (macro.top.mtype == M_INVOKE)
    {
        if ((! inAngleBrackets) && tokenSize[0] == 5)
            if (StrUcEqu("MACRO", tokStart[0])) {    // nested macro definition
                yyType = K_MACRO;
                PopToken();
    ...
    if (mSpoolMode & 1) {
        if (--macroSpoolNestDepth == 0) {       // Reached end of spooling ?
            ...
            if (! (macro.top.mtype == M_BODY)) {
                ...
                for (p = startMacroLine; p <= startMacroToken - 1; p++) { // plm reuses aVar
                    curChar = *p;
                    if (! IsWhite())
                        SyntaxError();
                }
                ...
                *macroInPtr = ESC;
                ...
                WriteM();
                symHighMark = (byte *)(endSymTab[TID_MACRO] = symTab[TID_MACRO]);      // reset macro parameter symbol table
...
void DoIterParam()
{
    if (savedTokenIdx + 1 != tokenIdx)
        ...
        baseMacroTbl = AddMacroText(tokStart[0], tokStart[0] + tokenSize[0] - 1);
        PopToken();

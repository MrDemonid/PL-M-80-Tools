...
#define MAXLL    122
static char _commandLine[MAXFL + 1];
static char *_commandLinePtr;
pointer MEMORY;
#define AVAILMEM    (0x9000+MAXFL)
...
int main(int argc, char **argv)
{
    ...
    memset(_commandLine, 0, sizeof(_commandLine));
    ...
    // определяем, откуда будем получать параметры
    if (argc > 1)
        s = argv[1];
    if (*s == '@')
    {
        ...
        s++;
        if (MapFile(&osfile, s) != ERROR_SUCCESS)
        {
        ...
        strcat(_commandLine, " ");
        size = MAXFL - strlen(_commandLine) - 4;
        i = fread(&_commandLine[strlen(_commandLine)], 1, size, f);
        fclose(f);
        if (i == size)
        {

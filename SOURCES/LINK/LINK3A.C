...
void ParseCmdLine()
{
    ...
    membot = MEMORY;                    // membot = malloc(AVAILMEM == 0x9000)
    topHeap = MemCk();                  // topHeap = MEMORY + AVAILMEM - 1
    if (High(MemCk() - MEMORY) - 1 >= 0x40)
        npbuf = 3072;
    else
        npbuf = 1056;
    sbufP = GetLow(npbuf);              // sbufP = topHeap - npbuf
    ebufP = bufP = sbufP + npbuf;       // ebufP = bufP = topHeap;
    scmdP = GetHigh(0);                 // scmdP = membot;
